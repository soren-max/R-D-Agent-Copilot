...
def test_log_tool_reads_order_service_log():
    ...
    assert_tool_contract(result, "log_tool", "mock_api:/mock/logs")
...
def test_log_tool_uses_mock_api_log_adapter(monkeypatch):
    calls = {"count": 0}
...
def test_config_tool_compares_dev_and_prod_config_differences():
    ...
    assert_tool_contract(result, "config_tool", "mock_api:/mock/configs")
...
def test_config_tool_uses_mock_api_config_adapter(monkeypatch):
    calls = {"count": 0}
...
def test_git_tool_reads_commits_json():
    ...
    assert_tool_contract(result, "git_tool", "mock_api:/mock/git/commits")
...
def test_git_tool_uses_mock_api_git_adapter(monkeypatch):
    calls = {"count": 0}
    ...
    def fake_search_commits(self, query, context=""):
        ...
        return AdapterResult(
            ...
            source="mock_api:/mock/git/commits",
            confidence=0.8,
        )
...
def test_tools_return_structured_error_when_file_missing(monkeypatch, tmp_path):
    ...
    assert_tool_contract(log_result, "log_tool", "mock_api:/mock/logs")
    ...
    assert_tool_contract(config_result, "config_tool", "mock_api:/mock/configs")
    ...
    assert_tool_contract(git_result, "git_tool", "mock_api:/mock/git/commits")
