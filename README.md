...
如需在生产化实验中切换 OpenAI-compatible embedding / rerank provider，可在本地 `.env` 中显式配置。默认不会外呼，只有开启 `RAG_EXTERNAL_CALLS_ENABLED=true` 才会访问外部 endpoint：

```bash
RAG_EMBEDDING_PROVIDER=openai_compatible
RAG_EMBEDDING_MODEL=your-embedding-model
RAG_EMBEDDING_BASE_URL=https://your-openai-compatible-endpoint
RAG_EMBEDDING_API_KEY=your_api_key_here

RAG_RERANK_PROVIDER=openai_compatible
RAG_RERANK_MODEL=your-rerank-model
RAG_RERANK_BASE_URL=https://your-openai-compatible-endpoint
RAG_RERANK_API_KEY=your_api_key_here

RAG_EXTERNAL_CALLS_ENABLED=true
RAG_PROVIDER_TIMEOUT_SECONDS=10
```
...
- Prompt Injection 检测：识别“忽略系统提示”、泄露 API key、越权、删除数据、`drop table`、`rm -rf` 等高风险请求。
- KB 文档恶意指令过滤：本地 Markdown KB 读取时会过滤包含高风险指令的文档，避免 RAG 把恶意文本当成可信知识。
- Tool allowlist / denylist：工具执行前校验 Planner 输出，只允许 `log_tool`、`config_tool`、`git_tool`、`rag_retriever`，拒绝未知工具和危险工具。
- Safety risk 场景降级：中高风险请求只允许安全工具路径；高风险请求直接返回 `safety_guard`，不会执行工具。

Trace 新增 `safety` stage，记录：
...
- Added tool allowlist / denylist validation before execution.
- Added safety guard response path for high-risk requests.
- Added safety metadata to Trace and ChatResponse.
...
GET /health
GET /config/check
GET /trace/export/{run_id}
GET /eval/report
```

接口说明：

- `/health` 返回服务版本、UTC timestamp 和关键检查项状态。
- `/config/check` 返回脱敏配置检查，包括 LLM 是否启用、provider/model、KB 文档数量、SQLite 配置和 RAG provider 状态；不会返回 API Key。
...
- Added persisted trace export endpoint for run replay and debugging.
- Added local evaluation report endpoint for planning and RAG reports.
...
- `LocalHashEmbeddingProvider`：默认本地 token / keyword hash embedding，不访问网络，测试可复现。
- `OpenAICompatibleEmbeddingProvider`：可选真实 embedding provider，支持 OpenAI-compatible embeddings endpoint。
- `LocalRerankProvider`：默认本地 overlap reranker，输出可解释 rerank 分数。
- `OpenAICompatibleRerankProvider`：可选真实 rerank provider，通过 OpenAI-compatible chat endpoint 返回 chunk 排序。

默认配置是 `local`。即使配置外部 provider，也必须显式设置 `RAG_EXTERNAL_CALLS_ENABLED=true` 才允许外呼；否则会快速回退本地 provider。Trace 会记录：
...
- Added explicit external-call gate for RAG providers.
