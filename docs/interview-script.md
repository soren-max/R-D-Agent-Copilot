...
R&D Agent Copilot 是一个面向研发排障场景的 AI Agent Harness。它不是普通聊天套壳，而是把一次用户请求拆成 Router、Planner、LangGraph Executor、Tool Gateway、Tools / RAG、Trace、Context Manager、Answer Synthesizer、Evaluation v2 和 Evidence Chain。系统可以回答配置中心这类知识问题，也可以模拟排查订单接口 500，通过日志、配置、Git 变更和知识库证据生成中文排障报告。

## 1 分钟版本

这个项目解决的是研发排障中信息分散、链路长、结论难复现的问题。后端用 FastAPI 暴露 `/chat`，请求先进入 Router 判断 simple_qa 或 complex_troubleshooting，再由 Planner 生成结构化步骤。Executor 内部使用 LangGraph 编排工具节点，并通过 Tool Gateway 调用本地 log、config、git 和 rag 工具收集证据。Trace 会记录每个阶段的输出、耗时、工具调用、跳过节点、provider metadata 和 fallback 状态。Context Manager 把 route、plan、tool evidence、RAG evidence、Incident Memory 和 run history 组织成有预算的上下文。DeepSeek 只作为最终 Answer Synthesizer，可选生成更自然的中文回答，失败时走 fallback。前端 Trace Viewer 展示执行链路，Evaluation v2 聚合工具、RAG、上下文、Memory、Resume、Provider 和 token/cost 指标。
...
Trace 和 Evaluation 是这个项目的另一个重点。Trace 让每次 Agent 执行可观察，前端 Trace Viewer 可以展示 Router、Planner、Executor、Context、Synthesizer、Evaluation 和 Evidence Chain 的完整链路。Evaluation v2 在回答生成后运行，用 rule-based 指标聚合工具成功率、Trace 完整性、RAG precision/recall/source coverage、上下文压缩、Memory freshness、Resume 状态、Provider fallback、token/cost 等信号。它不参与决策，只提供质量信号。

当前边界也很明确：项目使用本地样例数据模拟研发排障，不声称服务真实企业用户；没有接真实日志平台、配置中心或 Git API；默认 `LLM_ENABLED=false`，无 API Key 也能跑完整 fallback 链路。后续扩展方向是接真实 Adapter、继续收口 `app/` 与 `apps/api/app/` 目录、加入 LLM Judge 或人工 Review，并完善权限控制。
...
Evaluation v2 是 rule-based aggregation。它读取回答后的 route、plan、tool_results、trace、answer、provider metadata 和 RAG metrics，评估工具成功率、Trace 完整性、RAG precision/recall/source coverage、上下文压缩、Memory freshness、Resume 状态、Provider fallback、token/cost 等指标，不参与 Agent 决策。
