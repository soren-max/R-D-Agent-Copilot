...
function TimelineStep({ step, index }: { step: TraceStep; index: number }) {
  ...
  return (
    <div className={`rounded-2xl border border-slate-100 bg-white p-4 shadow-sm ${index > 0 ? "mt-3" : ""}`}>
      <div className={`border-l-2 ${stageColor(step.stage)} pl-4`}>
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {step.engine && (
              <span className={step.engine === "langgraph" ? "badge-green" : "badge-slate"}>
        ...
        {step.stage === "synthesizer" && (
          <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
            <span className={step.llm_used ? "badge-green" : "badge-slate"}>LLM {step.llm_used ? "是" : "否"}</span>
            {step.prompt_version && <span className="badge-blue">Prompt {step.prompt_version}</span>}
            {step.llm_error && <span className="badge-red">错误 {step.llm_error}</span>}
          </div>
        )}
        {step.stage === "evaluation" && (
          <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
            <span className="badge-green">评分 {typeof step.overall_score === "number" ? `${Math.round(step.overall_score * 100)}%` : "暂无数据"}</span>
            {step.evaluation_error && <span className="badge-red">错误 {step.evaluation_error}</span>}
          </div>
        )}
        <p className="mt-3 text-xs leading-5 text-slate-600">{step.output || "暂无数据"}</p>
...
function ToolCallBadge({ call }: { call: TraceToolCall }) {
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-100 bg-slate-50/70 px-3 py-2 text-xs">
...
function SkippedBadge({ node }: { node: TraceSkippedNode }) {
  return (
    <div className="flex items-center gap-2 rounded-xl border border-dashed border-slate-200 bg-slate-50/50 px-3 py-1.5 text-xs text-slate-500">
  ...
  return (
    <section className="card">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-slate-950">Trace Viewer Pro</h2>
          <p className="mt-1 text-xs text-slate-500">Router、Planner、LangGraph、Tools、Synthesizer 与 Evaluation 的执行链路。</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400">
          <span className="rounded-full bg-slate-100 px-2.5 py-1 font-mono">ID: {trace?.trace_id || "暂无数据"}</span>
          <span className="badge-slate">{steps.length} 阶段</span>
        </div>
      </div>
      </div>
